...
include "config.php";
    ...
    switch($page){
        ...
        case "author.php":
            if(isset($_GET['id'])){
                $sql_title = "SELECT first_name, last_name FROM users WHERE user_id = {$_GET['id']}";
                $result_title = mysqli_query($conn,$sql_title) or die("Query Failed");
                $row_title = mysqli_fetch_assoc($result_title);
            ...
            <?php
                ...
                if(mysqli_num_rows($result) > 0) { 
                     while($row = mysqli_fetch_assoc($result)){
                        if($row['logo'] == ""){
                            echo '<h1><a href="'.$hostname.'">'.$row['website_name'].'</a></h1>';
                        }else {
                            echo '<a href="'.$hostname.'"><img class="logo" src="admin/images/'.$row['logo'].'"></a>';
                        }
                    }
                }
            ?>
